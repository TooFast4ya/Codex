...
This bot has not PREFLIX

THIS BOT WAS MADE FOR A COLLEGE CLASS.

This is the Discord bot with no Ardunio Modules
...
client.on('ready', () => {
  ...
  client.user.setActivity('C0dex is My name.');
...
client.on('message', message => {
  ...
  if (message.content === 'filter pic') {
    number = 11; // Number of images make sur u check or will not work  
...
message.awaitReactions(filter, { max: 1, time: 60000, errors: ['time'] })
  ...
  client.on('message', message => {   // You could hook this to a API
